...
class Net(nn.Module):
    def __init__(self, tau, T, v_threshold=1.0, v_reset=0.0, mode='latency'):
        ...
        self.conv = layer.MultiStepContainer(
        ...
        self.fc = layer.MultiStepContainer(
            nn.Flatten(),
        ...
        self.out_layer = neuron.IFNode(v_threshold=Readout_v_threshold, v_reset=v_reset, surrogate_function=surrogate.ATan(), monitor_state=False)

    def forward(self, x):
        x_seq = self.static_conv(x).unsqueeze(0).repeat(self.T, 1, 1, 1, 1)
        x_seq = self.fc(self.conv(x_seq))

        out_spikes_counter = self.out_layer(x_seq[0])
        out_potential_counter = self.out_layer.v
        latency_score = self.surrogate_function(out_spikes_counter-0.5)
        for t in range(1, self.T):
            out_spikes_counter = out_spikes_counter + self.out_layer(x_seq[t])
            out_potential_counter = out_potential_counter + self.out_layer.v
