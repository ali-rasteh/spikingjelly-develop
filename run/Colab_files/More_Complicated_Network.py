...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # reg_loss_list[1][0] = (self.conv[3].spike.sum() / (np.prod(self.conv[3].spike.shape)))
        # reg_loss_list[2][0] = (self.fc[3].spike.sum() / (np.prod(self.fc[3].spike.shape)))
        # reg_loss_list[3][0] = (self.fc[6].spike.sum() / (np.prod(self.fc[6].spike.shape)))
        ...
        for t in range(1, self.T):
            ...
            # reg_loss_list[1][t] = (self.conv[3].spike.sum() / (np.prod(self.conv[3].spike.shape)))
            # reg_loss_list[2][t] = (self.fc[3].spike.sum() / (np.prod(self.fc[3].spike.shape)))
            # reg_loss_list[3][t] = (self.fc[6].spike.sum() / (np.prod(self.fc[6].spike.shape)))
