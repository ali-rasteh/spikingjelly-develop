...
class Net(nn.Module):
    def __init__(self, tau, T, v_threshold=1.0, v_reset=None, Readout_mode='latency', warmup=False):
        ...
        self.T = T
        self.deafult_T = 8
        self.surrogate_function = surrogate.ATan(spiking=False)
        ...
        self.fc = nn.Sequential(
            nn.Conv2d(16, 10, kernel_size=3, padding=1, bias=False),
            multiply(multiplier=1.0),
            neuron.OneSpikeIFNode(v_threshold=v_threshold, v_reset=v_reset, surrogate_function=self.surrogate_function, detach_reset=True, monitor_state=True),
            layer.FirstSpikePool2d(kernel_size=2, stride=2, padding=0, dilation=1, return_indices=False, ceil_mode=False),
    ...
    def forward(self, x):
        ...
        latency_score = torch.amax(latency_score, dim=(2,3))
...
def plot_spikes(net, data_loader, device, nb_plt, batch_size):
    ...
    #         spike_array = np.squeeze(np.reshape(np.asarray(module.monitor['s']), (:,:,:,14*14)))[:, 0, 0, :, :]
    #         s_t_array = spike_array.T  # s_t_array[i][j]表示神经元i在j时刻释放的脉冲，为0或1
    #         visualizing.plot_1d_spikes(spikes=s_t_array, title='Spikes of Neurons', xlabel='Simulating Step',
    #                                    ylabel='Neuron Index', int_x_ticks=True, int_y_ticks=True,
    #                                    plot_firing_rate=True, firing_rate_map_title='Firing Rate', dpi=200)
